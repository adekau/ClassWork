...
_scheduler *scheduler_init (_process *proc_arr) {
  ...
  scheduler->proc_arr[0] = *process_init("Process1", &(scheduler->created));
...
void scheduler_begin (_scheduler *scheduler) {
  ...
  char pname[50];		// For generating the process names.
  ...
  while(scheduler->n > 0) {
    ...
    if (scheduler->created < NUM_PROCESSES) {
      sprintf(pname, "Process%d", scheduler->created + 1);
      scheduler->proc_arr[scheduler->created] = *process_init(pname, &(scheduler->created));
    }
